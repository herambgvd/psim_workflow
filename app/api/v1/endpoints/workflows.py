...
REST API endpoints for workflow definition management including
CRUD operations, validation, activation/deactivation, and statistics.
"""
...
async def create_workflow(
        ...
        # current_user: Optional[UUID] = Depends(get_current_user)  # TODO: Add when auth is integrated
) -> WorkflowResponse:
    """
    ...
    """
    try:
        logger.info("Creating workflow via API", workflow_name=workflow_data.name)
        ...
        logger.info("Workflow created successfully via API", workflow_id=workflow.id)
        return workflow

    except WorkflowValidationError as e:
        logger.warning("Workflow validation failed", error=str(e))
        raise HTTPException(
        ...
        logger.error("Unexpected error creating workflow", error=str(e), exc_info=True)
        raise HTTPException(
...
async def list_workflows(
    ...
    try:
        ...
        workflows = await workflow_service.get_workflows(db=db, params=params)
...
async def get_workflow(
    ...
    try:
        ...
        logger.debug("Workflow retrieved successfully", workflow_id=workflow_id)
        return workflow
        ...
        logger.error("Error retrieving workflow", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_workflow(
    ...
    """
    try:
        logger.info("Updating workflow via API", workflow_id=workflow_id)

        # TODO: Extract user ID from authentication when integrated
        updated_by = None  # current_user
        ...
        logger.info("Workflow updated successfully via API", workflow_id=workflow_id)
        return workflow
        ...
        logger.warning("Workflow validation failed during update", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Workflow validation failed: {str(e)}"
        )
    except WorkflowServiceError as e:
        logger.error("Workflow update failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
    except Exception as e:
        logger.error("Unexpected error updating workflow", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_workflow(
    ...
    """
    try:
        logger.info("Deleting workflow via API", workflow_id=workflow_id, hard_delete=hard_delete)

        # TODO: Extract user ID from authentication when integrated
        deleted_by = None  # current_user
        ...
        logger.info("Workflow deleted successfully via API", workflow_id=workflow_id)

    except WorkflowNotFoundError:
        logger.warning("Workflow not found for deletion", workflow_id=workflow_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Workflow with ID {workflow_id} not found"
        )
    except WorkflowServiceError as e:
        logger.error("Workflow deletion failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
    except Exception as e:
        logger.error("Unexpected error deleting workflow", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def validate_workflow(
    ...
    """
    try:
        logger.debug("Validating workflow definition via API")
        ...
        logger.error("Error during workflow validation", error=str(e), exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def activate_workflow(
    ...
    """
    try:
        logger.info("Activating workflow via API", workflow_id=workflow_id)

        # TODO: Extract user ID from authentication when integrated
        activated_by = None  # current_user
        ...
        logger.info("Workflow activated successfully via API", workflow_id=workflow_id)
        return workflow

    except WorkflowNotFoundError:
        logger.warning("Workflow not found for activation", workflow_id=workflow_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Workflow with ID {workflow_id} not found"
        )
    except WorkflowServiceError as e:
        logger.error("Workflow activation failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
    except Exception as e:
        logger.error("Unexpected error activating workflow", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def deactivate_workflow(
    ...
    """
    try:
        logger.info("Deactivating workflow via API", workflow_id=workflow_id)

        # TODO: Extract user ID from authentication when integrated
        deactivated_by = None  # current_user
        ...
        logger.info("Workflow deactivated successfully via API", workflow_id=workflow_id)
        return workflow

    except WorkflowNotFoundError:
        logger.warning("Workflow not found for deactivation", workflow_id=workflow_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Workflow with ID {workflow_id} not found"
        )
    except WorkflowServiceError as e:
        logger.error("Workflow deactivation failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
    except Exception as e:
        logger.error("Unexpected error deactivating workflow", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_workflow_statistics(
    ...
    """
    try:
        logger.debug("Getting workflow statistics via API")

        stats = await workflow_service.get_workflow_stats(db=db)

        logger.debug("Workflow statistics retrieved successfully")
        return stats

    except Exception as e:
        logger.error("Error getting workflow statistics", error=str(e), exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_workflow_definition(
    ...
    try:
        ...
        logger.debug("Workflow definition retrieved successfully", workflow_id=workflow_id)
        ...
        logger.error("Error retrieving workflow definition", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def clone_workflow(
    ...
    """
    try:
        logger.info("Cloning workflow via API", workflow_id=workflow_id, new_version=new_version)
        ...
        logger.info("Workflow cloned successfully via API",
        ...
        logger.warning("Workflow not found for cloning", workflow_id=workflow_id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Workflow with ID {workflow_id} not found"
        )
    except WorkflowServiceError as e:
        logger.error("Workflow cloning failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
    except Exception as e:
        logger.error("Unexpected error cloning workflow", workflow_id=workflow_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
